...
exports.index = function(req, res) {
    if(req.session.user) {
        var query = Game.find({isnew:true}).populate('type', 'name flag').populate('author', 'name');        
    ...
    User.find({}, function(err, results) {
        ...
        if(results.length < 1) {
            ...
            if(req.method === 'GET') {
                ...
                var gametypes = [
                    {name:'休闲', flag:'arder'},
                    {name:'知识', flag:'knowledge'},
                    {name:'益智', flag:'puzzle'},
                    {name:'敏捷', flag:'agile'},
                    {name:'换装', flag:'facelift'},
                    {name:'棋牌', flag:'chess'},
                    {name:'策略', flag:'strategy'},
                    {name:'闯关', flag:'passlevel'}
                ];
                for (var i = 0; i < gametypes.length; i++) {                    
                    var gametype = new GameType(gametypes[i]);
                    gametype.save();
                }
